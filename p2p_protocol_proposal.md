...
2.  **Routing and Packet Structure**
     ...
     Packets are arranged like so, separated by a pre-defined flag:
         ...
         Pathfinding header - [broadcast, waterfall, whisper]
     ...
     ID construction is defined in section 5.
...
5.  **ID Constructions**
     ...
     2. Protocol IDs
         ...
         The version is defined in the code itself. As of this document's last update the version is "0.1.7".
...
8.  **Asynchronous Requests** (alpha)
     ...
     Then you place this in a dictionary so you can watch when this is responded to. A peer who gets this will reply:
         ...
         response
         [the id you assigned]
         [the json-ized address of the desired peer]
         End of tx
...
9.	**Potential Flaws**
     ...
     Second, there is quite a lot of extra data being sent. Using the default parameters, if you want to send a 4 character message it will be expanded to 184 characters. That's 46x larger. If you want these differences to be negligble, you need to send messages on the order of 512 characters. Then there is only an increase of ~36% (0% with decent compression). This can be improved by reducing the size of the various IDs being sent, or making the packet separators shorter. Both of these have disadvantages, however.

     Results using opportunistic compression look roughly as follows:
